<?php
//<NAM000>Customize layout
//<NAM001>Display PHP Info
//<NAM002>Pi CPU temperature
//<NAM003>External login method
//<ANH004>Creating a custom login page
...
//Application Title
define('APP_TITLE', 'Portfolio Manager');
...
//    'user' => '$2y$10$Fg6Dz8oH9fPoZ2jJan5tZuv6Z4Kp7avtQ9bDfrdRntXtPeiMAZyGO' //12345
    'huuanhh' => '$2y$10$mJCxl4DAMajzNm4VrG6O0u4iLxqrz1GlY3PPrxZeO0hGiSRT5gzfm', //CvkK825W
//-----<NAM003>
);
...
//+++++<NAM003> Moved to after authentication
...
//    if (isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']])) {
    ...
    if (isset($_SESSION[FM_SESSION_ID]['logged'])) {
        ...
        if (isset($auth_users[$user])) { // internal user
            // don't change $root_path
            $logged = true;
        }
        else if (is_dir($root_path.'/~'.$user)) { // external user
            $logged = true;
            $root_path = $root_path.'/~'.$user;
            $root_url = $root_url.'~'.$user;
        }
    }
    if ($logged) {
//-----<NAM003>
...
//                $_SESSION[FM_SESSION_ID]['logged'] = $_POST['fm_usr'];
            $user = $_POST['fm_usr'];
            if (isset($auth_users[$user]) && password_verify($_POST['fm_pwd'], $auth_users[$user])) { // internal user
                $logged = true;
            }
            else {
                $user = preg_replace('([^\w\s\d\-_~,;\[\]\(\).])', '', $user);
                $user = preg_replace('([\.]{2,})', '', $user);
                $pwdfile = $root_path.'/~'.$user.'/fm_pwd.php';
                if (is_readable($pwdfile)) {
                    @include($pwdfile);
                }
                if (isset($p) && password_verify($_POST['fm_pwd'], $p)) {
                    $logged = true;
                }
            }
            if ($logged) {
...
//-----<NAM003>
        ...
        <section class="bg-img">
            <div class="content">
                <header><?php echo APP_TITLE; ?></header>
                <form action="" method="post" autocomplete="off">
                    <div class="field">
						<span class="fa fa-user"></span>
                        <input type="text" id="fm_usr" name="fm_usr" required autofocus placeholder="Who are you?">
					</div>
                    <div class="field space">
						<span class="fa fa-lock"></span>
                        <input type="password" class="pass-key" id="fm_pwd" name="fm_pwd" required placeholder="Password">
...
<?php //+++++<NAM001>  ?>
                            <a title="<?php echo lng('PHP Info') ?>" class="dropdown-item nav-link" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;phpinfo=1"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> <?php echo lng('PHP Info') ?></a>
<?php //-----<NAM001> ?>
...
function lng($txt) {
    ...
    $tr['en']['AppName']        = 'Portfolio Manager';      $tr['en']['AppTitle']           = 'File Manager';
    //-----<ANH004>
